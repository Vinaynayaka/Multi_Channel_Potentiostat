...
def send_and_read(board, v_electrochem):
    ...
    Returns
    -------
    v_a0 : float — RE voltage (physical, 0-5V)
    v_a2 : float — TIA voltage (physical, 0-5V)
    """
    ...
    for _ in range(ADC_AVERAGE):
        re_sum  += board.chan_re.voltage
        tia_sum += board.chan_tia.voltage

    v_a0 = re_sum  / ADC_AVERAGE
    v_a2 = tia_sum / ADC_AVERAGE
